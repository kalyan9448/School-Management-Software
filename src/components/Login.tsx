...
export function Login() {
  ...
  const handleForgotPassword = async (e: React.FormEvent | any) => {
    e?.preventDefault?.();
    setError('');
    setLoading(true);

    // Send Firebase password reset email directly.
    // Firebase will silently handle non-existent addresses, so we don't need
    // to do a checkEmail guard here (it would block admins without school_id).
    try {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="p-8">
            ...
            {/* ════════════════════════════════════════════
                STEP 3b — Recovery Email Sent Confirmation
            ════════════════════════════════════════════ */}
            {step === 'reset-password' && (
              <div className="space-y-5">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-green-700 leading-relaxed">
                        A password reset link has been sent to <span className="font-medium">{email}</span>. Check your inbox (and spam folder) and click the link to set a new password.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-3 bg-amber-50 border border-amber-100 rounded-lg text-xs text-amber-800">
                  <p>⚠️ The link expires in <strong>1 hour</strong>. If you don't see the email, check your spam folder or request a new one.</p>
                </div>
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-2 border border-purple-300 text-purple-700 py-2.5 rounded-lg text-sm font-medium hover:bg-purple-50 transition-all disabled:opacity-50"
                >
                  {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Resend Recovery Email'}
                </button>
